...
from mtools.models import BaseModel
...
from mtools import set_seed, data_filter_remove_fr, total_counter, DataLoaderGenerator, data_filter_remove_fd, CombineFilters
from mtools import get_loss_fig
from mtools.display import dict_str, get_error_distrubition_fig, get_error_stats, get_loss_row, get_stats_row, plot_stats
...
train_params['num_epochs'] = [5]             # Number of epochs for training
...
if __name__ == "__main__":
    ...
    with tqdm(total=total, desc="Sweeping") as pbar:
                    ...
                    try:
                        ...
                        for model_training_params_tuple in itertools.product(*[model_params[key] for key in model_params.keys()], *[train_params[key] for key in train_params.keys()]): # Maybe remove num_epochs and have ongoing training?
                            ...
                            try:
                                ...
                                # Train the model
                                model.run_training(optimizer, train_loader, val_loader, num_epochs, verbose=True)
                                ...
                                stats_df.loc[iter] = get_stats_row(train_stats, val_stats)
                                loss_df.loc[iter] = get_loss_row(loss_func)
        ...
        # Save the report
        md["Error Figure"].add_image(plot_stats(stats_df))
        md.save()
        # report.add_image_report("Error Figure", plot_stats(stats_df))
        # report.save_report()
